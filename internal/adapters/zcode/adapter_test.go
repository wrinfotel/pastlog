...
const (
	parentID = "sess_fixture000001zc"
	childID  = "sess_fixture000002zc"
)

func TestDetect(t *testing.T) {
	if NewDir(t.TempDir()).Detect() {
		t.Fatal("Detect should be false without db.sqlite")
	}
	...
	if got, want := filepath.ToSlash(a.StoragePath()), filepath.ToSlash(a.dir); got != want {
		t.Errorf("StoragePath = %q, want %q (the storage dir)", got, want)
	}
	// a directory named db.sqlite is not a database
	dir := t.TempDir()
	if err := os.MkdirAll(filepath.Join(dir, "db.sqlite"), 0o755); err != nil {
		t.Fatal(err)
	}
	if NewDir(dir).Detect() {
		t.Error("Detect should be false when db.sqlite is a directory")
	}
}

// TestDetectViaHome pins the discovery wiring: New(home) resolves
// <home>/.zcode/cli/db (discovery.ZCodeDir).
func TestDetectViaHome(t *testing.T) {
	home := t.TempDir()
	if New(home).Detect() {
		t.Fatal("Detect should be false in an empty home")
	}
	...
	a := New(home)
	if !a.Detect() {
		t.Fatal("Detect should be true with ~/.zcode/cli/db/db.sqlite present")
	}
	if got, want := filepath.ToSlash(a.StoragePath()), filepath.ToSlash(dir); got != want {
...
func TestSessionsRealisticFixture(t *testing.T) {
	...
	if parent.Project != "C:/dev/fixture zcode" {
...
func TestSessionSizeSumsRowData(t *testing.T) {
	...
	db, err := sql.Open("sqlite", "file:"+filepath.ToSlash(filepath.Join(dir, "db.sqlite"))+"?mode=ro")
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()
...
func TestEntriesStreamRealistic(t *testing.T) {
	...
	wants := []want{
		...
		{agentlog.ToolCall, "tool", `{"file_path":"C:/dev/fixture app/flux.ts"}`},
		{agentlog.ToolResult, "tool", "const flux = calibrate(0.42)"},
		{agentlog.ToolCall, "tool", `{"file_path":"flux.ts"}`}, // pending tool: no result yet
	}
	...
	if want := time.UnixMilli(1786480128154); !got[0].Timestamp.Equal(want) {
...
func TestEntriesSkippedRecordsCounted(t *testing.T) {
	...
	// (file, timeline, step-start, step-finish, compaction) are dropped
...
func TestEntriesUnknownSessionErrors(t *testing.T) {
	a := NewDir(buildFixtureDB(t))
	err := a.Entries(agentlog.Session{ID: "sess_nope"}, func(e agentlog.Entry) error { return nil })
	if err == nil {
...
func lockDatabase(t *testing.T, dir string) func() {
	t.Helper()
	dsn := "file:" + filepath.ToSlash(filepath.Join(dir, "db.sqlite"))
