...
// version-dependent.
type partRaw struct {
	Type  string        `json:"type"` // text | reasoning | tool | file | timeline | step-start | step-finish | compaction | …
...
// output a plain (possibly large) string.
...
// types. The recognized-but-unmapped types (file, timeline, step-start,
...
func partToEntries(role, data string, ts time.Time) (entries []agentlog.Entry, skip bool) {
	...
	default:
		// recognized-but-unmapped (file, timeline, step-start, step-finish,
...
var recognizedPartTypes = map[string]bool{
	...
	"tool":        true,
	"file":        true,
	"timeline":    true, // zcode-specific UI timeline record
	"step-start":  true,
	"step-finish": true,
	"compaction":  true,
}
