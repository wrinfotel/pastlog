// Package zcode adapts ZCode's SQLite storage: a single db.sqlite (with
// -wal/-shm side files) under <home>/.zcode/cli/db (see SCHEMA.md and
// internal/discovery.ZCodeDir). The database is opened strictly read-only;
// iteration streams per-session SQL cursors, never materializing whole result
// sets. If the database is locked by a running ZCode instance the adapter
// degrades to unavailable with one warning while other agents continue
// (spec §4).
//
// Unlike opencode (whose session table carries aggregate token columns),
// ZCode reports usage per model request in a separate model_usage table, so
// the usage view aggregates that table per session.
package zcode
...
// Adapter reads ZCode sessions from the ~/.zcode/cli/db directory.
type Adapter struct {
	dir     string // resolved storage dir, "" when absent
...
// ZCode storage dir via discovery.
func New(home string) *Adapter {
	return &Adapter{dir: discovery.ZCodeDir(home)}
}

// NewDir builds an adapter for an explicit storage dir (tests, exotic setups).
...
// StoragePath implements agentlog.PathSource: the storage dir scanned for
...
func (a *Adapter) Warning() string {
	...
	return "zcode: database is locked by a running zcode instance — skipping it; " +
		"other agents are unaffected (retry after closing zcode)"
}
...
// SessionsUsage implements agentlog.UsageSource: ZCode reports tokens per
// model request in the model_usage table, so the usage view aggregates that
// table per session and rides the same walk as SessionsMeta (one connection,
// one scan, locked-DB handling included).
...
// per-session sizes, message counts and the token/model aggregates from
// model_usage — the shared walk behind Sessions/SessionsMeta (meta view) and
// SessionsUsage (usage view), so all listing passes open one connection and
// scan the session table once.
func (a *Adapter) walkUsage(iter func(agentlog.SessionUsage) error) error {
	if a.dir == "" {
		return nil // storage dir absent: no sessions, not an error (spec §8)
	}
	db, err := a.open()
	if err != nil {
		if a.markLocked(err) {
		...
		return fmt.Errorf("cannot open zcode database: %v", err)
	}
	defer db.Close()
	...
	if err != nil {
		...
		return fmt.Errorf("cannot list zcode sessions: %v", err)
	}
	defer rows.Close()
...
// degradation for older databases), via the driver's message.
...
func (a *Adapter) Entries(s agentlog.Session, iter func(agentlog.Entry) error) error {
	if a.dir == "" {
		return nil // storage dir absent: nothing to read, not an error
	}
	db, err := a.open()
	if err != nil {
		if a.markLocked(err) {
	...
	if err := db.QueryRow(`SELECT 1 FROM session WHERE id = ?`, s.ID).Scan(&exists); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return fmt.Errorf("session %s not found in zcode storage", s.ID)
		}
		if a.markLocked(err) {
			return nil
		}
		return fmt.Errorf("cannot read zcode session %s: %v", s.ID, err)
	}
